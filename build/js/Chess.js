...
const findAIMove_1 = require("./findAIMove");
var Chess;
(function (Chess) {
    var _a;
    function Board() {
    ...
    function setEnPassantCol(flags, col) {
        flags &= ~masks.enPassantAll;
        flags |= masks.enPassant;
        flags |= col << 5;
        return flags;
    }
    function enPassantPos(board) {
        const col = enPassantCol(board);
        if (col === null) {
            return null;
        }
        const row = (board[64] === Chess.codes.sides.white ?
     ...
     */
    function* findPieces(board, pieceCodes) {
        for (let i = 0; i !== 64; i++) {
    ...
    _a = (() => {
        ...
        function toWhite(code) {
            return !isWhite(code) ? code + diff : code;
        }
        function* travelMoves(board, pos, [di, dj], isPlayerWhite) {
            if (di === 0 && dj === 0) {
                return;
            }
            for (let dist = 1; true; dist++) {
                ...
                if (newPosCode === Chess.codes.emptySquare) {
                    yield newPos;
                    continue;
                }
        ...
        function* rookMoves(board, pos, isPlayerWhite) {
            for (const unitMove of [
                [0, 1],
                [0, -1],
                [1, 0],
                [-1, 0],
        ...
        function* queenMoves(board, pos, isPlayerWhite) {
        ...
        function* pawnMoves(board, pos, isPlayerWhite) {
            ...
            for (const [di, dj] of forwardMoves) {
                ...
                const newPos = pos + 8 * di + dj;
                if (di === 2 * iDirection &&
                    board[pos + 8 * iDirection + dj] !== Chess.codes.emptySquare) {
                    continue;
                }
                if (board[newPos] === Chess.codes.emptySquare) {
                    yield newPos;
                }
            }
            for (const [di, dj] of [[iDirection, 1], [iDirection, -1]]) {
                ...
                const newPos = pos + 8 * di + dj;
                const newPosCode = board[newPos];
                if ((newPosCode !== Chess.codes.emptySquare && isWhite(newPosCode) !== isPlayerWhite) ||
                    newPos === enPassantPos(board)) {
                    yield newPos;
                }
            }
        }
        function* plainKingMoves(board, pos, isPlayerWhite) {
        ...
        function isKingInCheck(board, pos, isPlayerWhite) {
            const opponentPieces = (isPlayerWhite ?
                Chess.codes.pieces.black :
                Chess.codes.pieces.white);
        ...
        function* kingMoves(board, pos, isPlayerWhite) {
            ...
            if ((flags & castleMasks.castle0) &&
                board[i8 + 1] === Chess.codes.emptySquare &&
                board[i8 + 2] === Chess.codes.emptySquare &&
                board[i8 + 3] === Chess.codes.emptySquare &&
                !isKingInCheck(board, pos, isPlayerWhite) &&
                !isKingInCheck(board, i8 + 3, isPlayerWhite)) {
                yield i8 + 2;
            }
            if ((flags & castleMasks.castle7) &&
                board[i8 + 5] === Chess.codes.emptySquare &&
                board[i8 + 6] === Chess.codes.emptySquare &&
                !isKingInCheck(board, pos, isPlayerWhite) &&
                !isKingInCheck(board, i8 + 5, isPlayerWhite)) {
                yield i8 + 6;
            }
        }
        function* findMoves(board, pos) {
            const pieceCode = board[pos];
            if (pieceCode === '.'.charCodeAt(0)) {
                return;
            }
            const isPlayerWhite = isWhite(pieceCode);
            const pieces = Chess.codes.pieces.white;
            const moveGen = {
    ...
    })(), Chess.findMoves = _a.findMoves, Chess.isKingInCheck = _a.isKingInCheck, Chess.isWhite = _a.isWhite, Chess.toWhite = _a.toWhite;
    function applyMove(board, [from, to], promotionPreference = () => (Chess.isWhite(board[from]) ?
        Chess.codes.pieces.white.queen :
        Chess.codes.pieces.black.queen)) {
        const newBoard = Board.copy(board);
        let flags = board[65];
        // Update who's turn it is
        newBoard[64] = (board[64] === Chess.codes.sides.white ?
            Chess.codes.sides.black :
            Chess.codes.sides.white);
        // Move piece
        newBoard[from] = Chess.codes.emptySquare;
        newBoard[to] = board[from];
        const piece = Chess.toWhite(board[from]);
        const pieces = Chess.codes.pieces.white;
        ...
        if (piece === pieces.pawn) {
            ...
            if (dist === 2 * 8) {
            ...
            else if (((dist === 8 - 1 || dist === 8 + 1) && // Diagonal move
                board[to] === Chess.codes.emptySquare // Destination empty
            )) {
                ...
                newBoard[opponentPawnPos] = Chess.codes.emptySquare;
            }
            else {
                const toRow = row(to);
                if (toRow === 0 || toRow === 7) {
        ...
        const [castleMasks, homeRow] = (board[64] === Chess.codes.sides.white ?
        ...
        if (piece === pieces.king) {
            ...
            if (Math.abs(from - to) === 2) {
                ...
                newBoard[rookFrom] = Chess.codes.emptySquare;
    ...
    Chess.applyMove = applyMove;
    Chess.findAIMove = findAIMove_1.default;
})(Chess || (Chess = {}));
